...
public class SimConfigId {
    ...
    private String getMappingMatch(List<SimCombination> list, HashMap<String, String> simParams) {
        ...
        for (SimCombination simCombo : list) {
            int count = 0;
            if (simCombo.getMCC() != null && simCombo.getMCC().equals(simParams.get(MCC))
                    && simCombo.getMNC() != null && simCombo.getMNC().equals(simParams.get(MNC))) {
                CSLog.d(TAG, "getMappingMatch - mcc: " + simCombo.getMCC() + " mnc: " + simCombo.getMNC()
                        + " for: " + simCombo.getSimConfigId());
                count++;
            }
            if (simCombo.getServiceProvider() != null && matchOnSP(simCombo.getServiceProvider(), simParams.get(SP))) {
            ...
            if (simCombo.getGid1() != null &&
                    simParams.get(GID1) != null && simParams.get(GID1).toLowerCase().startsWith(simCombo.getGid1().toLowerCase())) {
